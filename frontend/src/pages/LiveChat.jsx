                    ...
                    {sessions.map(phone => (
                        <button
                            ...
                            className={`w-full flex items-center gap-3 p-4 rounded-2xl transition-all ${
                                selectedPhone === phone 
                                ? 'bg-primary/20 border border-primary/30 text-white shadow-lg' 
                                : 'text-slate-400 hover:bg-white/5 border border-transparent'
                            }`}
                        >
                            <div className="w-10 h-10 rounded-full bg-surface border border-white/10 flex items-center justify-center font-bold text-xs">
                                {phone.slice(-4)}
                            </div>
                    ...
                    <>
                            ...
                            {messages.map((msg, i) => (
                                ...
                                >
                                    <div className={`max-w-[75%] px-5 py-3.5 rounded-3xl shadow-lg relative ${
                                        ...
                                        : msg.sender === 'ai'
                                            ? 'bg-primary/20 border border-primary/20 text-white rounded-tr-none shadow-primary/5'
                                            : 'bg-accent/20 border border-accent/20 text-white rounded-tr-none'
                                    }`}>
                                        <div className="flex items-center gap-1.5 mb-1.5">
                                            {msg.sender === 'ai' && <Bot size={12} className="text-primary-foreground opacity-70" />}
                                            {msg.sender === 'agent' && <Shield size={12} className="text-accent opacity-70" />}
                        ...
                        <form onSubmit={handleSend} className="p-6 bg-black/20 border-t border-white/5 flex gap-3">
                            <input 
                                ...
                                className="flex-1 bg-white/5 border border-white/10 rounded-2xl px-6 py-4 outline-none focus:border-primary/50 transition-all text-sm placeholder:text-slate-600 font-medium" 
                        ...
                        {aiEnabled && (
                            <div className="absolute bottom-24 left-1/2 -translate-x-1/2 bg-surface/90 backdrop-blur-md border border-white/10 px-4 py-2 rounded-full text-[10px] font-bold text-slate-400 flex items-center gap-2 pointer-events-none shadow-2xl">
